...
public class MingxingFragment extends Fragment implements OnRefreshListener{
    private static final String TAG = "MingxingFragment";
    protected View mView; // 声明一个视图对象
    protected Context mContext; // 声明一个上下文对象
    private SwipeRefreshLayout srl_2; // 声明一个下拉刷新布局对象
    private RecyclerView rv_2; // 声明一个循环视图对象
    private RecyclerGridAdapter mAdapter; // 声明一个普通网格适配器对象
    private ArrayList<ImagesInfo> mAllArray; // 明星表情队列
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        srl_2 = mView.findViewById(R.id.srl);
        // 设置srl_2的下拉刷新监听器
        srl_2.setOnRefreshListener(this);
        // 设置srl2的下拉变色资源数组
        srl_2.setColorSchemeResources(
                R.color.red, R.color.orange, R.color.green, R.color.blue);
        // 从布局文件中获取名叫rv的循环视图
        rv_2 = mView.findViewById(R.id.rv);
        // 创建一个垂直方向的普通网格布局管理器
        GridLayoutManager manager = new GridLayoutManager(getActivity(), 3);
        // 设置循环视图的布局管理器
        rv_2.setLayoutManager(manager);
        // 获取默认的明星表情信息队列
        mAllArray = ImagesInfo.getDefault(1);
        // 构建一个明星表情列表的普通网格适配器
        ...
        // 给rv_2设置明星表情普通网格适配器
        rv_2.setAdapter(mAdapter);
        // 设置rv_2的默认动画效果
        rv_2.setItemAnimator(new DefaultItemAnimator());
        // 给rv_2添加列表项之间的空白装饰
        rv_2.addItemDecoration(new SpacesItemDecoration(3));
