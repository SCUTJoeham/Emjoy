...
public class DongmanFragment extends Fragment implements OnRefreshListener{
    private static final String TAG = " DongmanFragment";
    protected View mView; // 声明一个视图对象
    protected Context mContext; // 声明一个上下文对象
    private SwipeRefreshLayout srl_3; // 声明一个下拉刷新布局对象
    private RecyclerView rv_3; // 声明一个循环视图对象
    private RecyclerGridAdapter mAdapter; // 声明一个普通网格适配器对象
    private ArrayList<ImagesInfo> mAllArray; // 动漫表情队列
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        srl_3 = mView.findViewById(R.id.srl);
        // 设置srl_3的下拉刷新监听器
        srl_3.setOnRefreshListener(this);
        // 设置srl_3的下拉变色资源数组
        srl_3.setColorSchemeResources(
                R.color.red, R.color.orange, R.color.green, R.color.blue);
        // 从布局文件中获取名叫rv的循环视图
        rv_3 = mView.findViewById(R.id.rv);
        // 创建一个垂直方向的普通网格布局管理器
        GridLayoutManager manager = new GridLayoutManager(getActivity(), 3);
        // 设置循环视图的布局管理器
        rv_3.setLayoutManager(manager);
        // 获取默认的动漫表情信息队列
        mAllArray = ImagesInfo.getDefault(2);
        // 构建一个动漫表情列表的普通网格适配器
        ...
        // 给rv_3设置动漫表情普通网格适配器
        rv_3.setAdapter(mAdapter);
        // 设置rv_3的默认动画效果
        rv_3.setItemAnimator(new DefaultItemAnimator());
        // 给rv_3添加列表项之间的空白装饰
        rv_3.addItemDecoration(new SpacesItemDecoration(3));
